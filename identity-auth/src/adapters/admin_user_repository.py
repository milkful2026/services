...
is `JSONB` in the production-authoritative migration
(migrations/0001_admin_user.sql), matching the `JSON` column here —
exactly like that module's own `lines` column (`JSONB` in its
migration, `JSON` here). An earlier version of this file's comment
claimed the migration used `TEXT[]`; it did, briefly, and that was a
real bug (a genuine Postgres type mismatch that broke every insert —
SQLite's untyped columns never caught it) fixed by changing the
migration to `JSONB` to actually match this table definition, not by
changing this file.
