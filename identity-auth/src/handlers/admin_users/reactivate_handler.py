"""POST /v1/admin/users/{id}/reactivate — thin Lambda entrypoint (FR-4)."""
...
def handler(event: dict, context) -> dict:
    ...
    try:
        updated = deps["user_service"].reactivate_admin(caller["role"], target_id, correlation_id)
        return success_response(admin_to_dict(updated))
    except IdentityAuthError as exc:
        logger.info(
            "admin_reactivate_user rejected",
